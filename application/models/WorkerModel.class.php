<?php
	class WorkerModel extends Model
	{
		...
		{
			$sql = 'select pt.id, pt.publishTime, mi.code as modelCode, pi.code as partCode, pi.name as partName, pt.procedureName, mai.code as machineCode, mai.name as machineName from pending_task as pt left join model_info mi on pt.modelId=mi.id left join part_info pi on pt.partId=pi.id left join machine_info mai on pt.machineId=mai.id where pt.workerId=? and pt.state=1 and mai.isActive=1 order by pt.publishTime ASC';
		...
		{
			$sql = 'select pt.partId, pt.procedureName, pt.machineId, ui.lowWage, ui.highWage, ui.social from pending_task as pt left join user_info ui on pt.workerId=ui.id where pt.id=? and pt.state=1 and pt.workerId=?';
			...
			if(count($res) == 0){
				...
				$sql = 'insert into part_task_list (name, belongedPart, machineId, startTime, userId, state, lowWage, highWage, pendTaskId) values(?,?,?,?,?,?,?,?,?)';
				$time = time();
				if(!$this->execute($sql, array($res[0]['procedureName'], $res[0]['partId'], $res[0]['machineId'], $time, $userId, 0, floatval($res[0]['lowWage'])+floatval($res[0]['social']), $res[0]['highWage'], $taskId))){
					return false;
				}
				return true;
			}
		...
		public function endTask($taskId)
		{
			...
			$sql = 'update part_task_list set state=2, finishTime=?, userTime=?, machineTime=?, lowPayedTime=?, highPayedTime=? where id=?';
			if(!$this->execute($sql, array(floor($now), floor($userTime), floor($userTime), floor($lowPayedTime), floor($highPayedTime), $taskId))){
				return false;
			}
			return true;
		}
