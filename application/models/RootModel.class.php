<?php
	class RootModel extends Model
	{
		...
		{
			$sql = 'select ui.id, ui.name, ui.phoneNum, ui.lowWage, ui.highWage, ui.social, ui.deptId from user_info as ui left join dept_info di on ui.deptId=di.id where ui.isActive=1 and di.rightId<>1 order by ui.deptId';
			return $this->query($sql ,array());
		}

		public function getWorkerById($id)
		{
			$sql = 'select id, name, lowWage, highWage, phoneNum, deptId from user_info where isActive=1 and id=?';
			return $this->query($sql ,array($id))[0];
		}

		public function getWorkerByDeptId($id)
		{
			$sql = 'select id, name, lowWage, highWage, phoneNum, deptId from user_info where isActive=1 and deptId=?';
		...
		public function workerSave($id, $name, $lowWage, $highWage, $social, $phoneNum, $deptId)
		{
			...
			$sql = 'update user_info set name=?,lowWage=?,highWage=?,social=?,phoneNum=?,deptId=? where id=?';
			return $this->execute($sql, array($name, $lowWage, $highWage, $social, $phoneNum, $deptId, $id));
		}
		...
		public function workerAdd($id, $name, $lowWage, $highWage, $social, $phoneNum, $deptId)
		{
			$sql = 'select id from dept_info where id=? and isActive=1';
			$res = $this->query($sql, array($deptId));
			if(count($res) == 0){
			...
			$sql = 'insert into user_info (id,password,name,lowWage,highWage,social,phoneNum,deptId) values(?,?,?,?,?,?,?,?)';
			return $this->execute($sql, array($id, '123456', $name, $lowWage, $highWage, $social, $phoneNum, $deptId));
		}

		public function getMachine()
		{
			$sql = 'select id,code,name,belongedDept as deptId,depreFee,power,workType from machine_info where isActive=1 order by deptId';
			return $this->query($sql ,array());
		}

		public function getMachineByCodeAndName($code, $name)
		{
			$sql = 'select id,code,name,belongedDept as deptId,depreFee,power,workType from machine_info where isActive=1 and code=? and name=?';
			return $this->query($sql ,array($code, $name));
		}

		public function getMachineByDeptId($id)
		{
			$sql = 'select id,code,name,belongedDept as deptId,depreFee,power,workType from machine_info where isActive=1 and belongedDept=?';
			return $this->query($sql ,array($id));
		}
		...
		public function machineSave($id, $name, $depreFee, $power, $workWay, $deptId)
		{
			$sql = 'select id from dept_info where id=? and isActive=1';
			$res = $this->query($sql, array($deptId));
			if(count($res) == 0){
			...
			$sql = 'update machine_info set name=?,depreFee=?,power=?,workType=?,belongedDept=? where id=?';
			return $this->execute($sql, array($name, $depreFee, $power, $workWay, $deptId, $id));
		}
		...
		public function machineAdd($code, $name, $power, $depreFee, $workWay, $deptId)
		{
			$sql = 'select id from dept_info where id=? and isActive=1';
			$res = $this->query($sql, array($deptId));
			if(count($res) == 0){
			...
			$sql = 'insert into machine_info (code,name,power,depreFee,workType,belongedDept,isVirtual) values(?,?,?,?,?,?,?)';
			return $this->execute($sql, array($code, $name, $power, $depreFee, $workWay, $deptId, 0));
		}
		...
		public function modifyWorker($id, $lowWage, $highWage, $social)
		{
			...
			if(count($res) == 0){
				...
				$sql = 'update user_info set lowWage=?,highWage=?,social=? where id=?';
				return $this->execute($sql, array($lowWage, $highWage, $social, $id));
			}
		}
	}
?>