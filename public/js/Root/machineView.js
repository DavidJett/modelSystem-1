...
var modelSystemRootMachine = function(_dept){
	dept = _dept;
	table = $('#table');
...
var reloadTable = function(type, val){
	...
	if(type == 'code&name'){
		$.ajax({
			url: modelSystemURLHeader + '/Root/getMachineByCodeAndName',
			type:'POST',
			data:{'code':val[0],'name':val[1]},
			success: function(data){
				data = JSON.parse(data.trim());
				for(var i in data){
					addTr(data[i]['id'],data[i]['code'],data[i]['name'],data[i]['deptId'],data[i]['depreFee'],data[i]['power'],data[i]['workType']);
				}
			}
		});
	}else if(type == 'deptId'){
		$.ajax({
			url: modelSystemURLHeader + '/Root/getMachineByDeptId',
			type:'POST',
			data:{'id':val},
			success: function(data){
				data = JSON.parse(data.trim());
		...
		$.ajax({
			url: modelSystemURLHeader + '/Root/getMachine',
			type:'POST',
			data:{},
			success: function(data){
				data = JSON.parse(data.trim());
...
var addTr = function(id,code,name,deptId,depreFee,power,workType){
	var tr = $('<tr id="'+id+'"></tr>');
	var tdCode = $('<td class="code">'+code+'</td>');
	var tdName = $('<td class="name"><input style="width:270px" disabled="disabled" value="'+name+'"/></td>');
	var tdDepreFee = $('<td class="deprefee"><input disabled="disabled" value="'+depreFee+'"/></td>');
	var tdPower = $('<td class="power"><input disabled="disabled" value="'+power+'"/></td>');
...
var save = function(e){
	...
	var depreFee = target.find('.depreFee').find('input').val();
	var power = target.find('.power').find('input').val();
	var workWay = target.find('.workway').find('select').val();
	var deptId = target.find('.deptid').find('select').val();

	if(!depreFee.match(s)){
		alert('输入的月折旧必须是一个不小于零的数字');
		return;
	}
	if(!power.match(s)){
		alert('输入的功率必须是一个不小于零的数字');
		return;
	}
	$.ajax({
		url: modelSystemURLHeader + '/Root/machineSave',
		type:'POST',
		data:{'id':id, 'name':name, 'depreFee':depreFee, 'power':power, 'workWay':workWay, 'deptId':deptId},
		success: function(data){
			data = data.trim();
			if(data == 'success'){
				alert('保存成功');
...
var remove = function(e){
	...
	if(confirm('确定要删除机器: '+target.find('.code').html()+"?")){
		$.ajax({
			url: modelSystemURLHeader + '/Root/machineRemove',
			type:'POST',
			data:{'id':id},
			success: function(data){
				data = data.trim();
...
var addMachine = function(){
	...
	var depreFeeContainer = $('<div><p class="label">月折旧:</p></div>');
	...
	depreFeeContainer.append(depreFeeInput);
	...
	container.append(depreFeeContainer);
	...
	checkButton.on('click', function(){
		...
		var depreFee = depreFeeInput.val();
		...
		if(!depreFee.match(s)){
			notice.html('输入的月折旧必须是一个不小于零的数字');
		...
		$.ajax({
			url: modelSystemURLHeader + '/Root/machineAdd',
			type:'POST',
			data:{'code':code, 'name':name, 'power':power, 'depreFee':depreFee, 'workWay':workWay, 'deptId':deptId},
			success: function(data){
				data = data.trim();
				console.log(data);
				if(data == 'success'){
